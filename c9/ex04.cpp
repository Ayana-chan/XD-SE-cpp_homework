...
namespace Parser_Arithmometer
{
    using std::string;
    using std::istream;
    using Parser_BasicReader::Token;
    using Parser_BasicReader::get_token;
...
namespace Parser_Arithmometer
{
    ...
    void read_include_filename(istream* input)
    {
        ...
        char ch;
        if(tok.token == Parser_BasicReader::TOK_LT || tok.token == Parser_BasicReader::TOK_QUOT)
        {
            bool sysPath = tok.token == Parser_BasicReader::TOK_LT;
            string fileName = read_simple_string(input, sysPath ? Parser_BasicReader::TOK_GT : Parser_BasicReader::TOK_QUOT);
    ...
    {
        Token tok = get_token(input, true);
        if(tok.token == Parser_BasicReader::TOK_WORD && tok.value == "include")
        {
        ...
        for(;;)
        {
            ...
            if(tok.token == Parser_BasicReader::TOK_END || tok.token == Parser_BasicReader::TOK_NEWLINE)
                break;
        }
    }
    ...
    void read_comment(istream* input)
    {
        ...
        for(;;)
        {
            ...
            switch(tok.token)
            {
            case Parser_BasicReader::TOK_NEWLINE:
                if(!firstLine)
                    stats.commentLines++;
                firstLine = false;
                break;
            case Parser_BasicReader::TOK_END:
            case Parser_BasicReader::TOK_COMMENT_END:
                stats.commentLines++;
                return;
            default:
                break;
    ...
    void read_normal_line(istream* input, Token tok)
    {
        for(;;)
        {
            switch(tok.token)
            {
            case Parser_BasicReader::TOK_WHITESPACE:
                stats.numWords++;
                break;
            case Parser_BasicReader::TOK_COMMENT_START:
                read_comment(input);
                break;
            case Parser_BasicReader::TOK_NEWLINE:
            case Parser_BasicReader::TOK_END:
    ...
    {
        if(tok.token == Parser_BasicReader::TOK_NONE) tok = get_token(input, true);
        switch(tok.token)
        {
        case Parser_BasicReader::TOK_END:
        case Parser_BasicReader::TOK_NEWLINE:
            return;
        case Parser_BasicReader::TOK_POUND:
            read_directive(input);
            break;
        case Parser_BasicReader::TOK_COMMENT:
            stats.commentLines++;
    ...
    void read_file(string fileName)
    {
        ...
        while(file->good())
        {
            read_line(file, Token(Parser_BasicReader::TOK_NONE));
        }
        delete file;
    }
}
...
int main(int argc, char* argv[])
{
    ...
    Parser_Arithmometer::init();
    Parser_Arithmometer::read_file(argv[1]);
}
